...
└── palettes/
    ...
    └── [palette].ts            # atlas, blueprint, catppuccin, gruvbox, nord, rose-pine, slate, solarized, tidewater, tokyo-night
```
...
10 palettes (atlas, blueprint, catppuccin, gruvbox, nord, rose-pine, slate, solarized, tidewater, tokyo-night), each with light/dark/transparent themes. `color-utils.ts` provides HSL conversion and a `mix()` helper for blending colors.
