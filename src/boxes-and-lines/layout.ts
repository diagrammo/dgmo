...
export async function layoutBoxesAndLines(
  ...
  const searchOpts = {
    ...
    // Spread into the label-reserving relayout below too, via `...searchOpts`:
    // that second search has its OWN clock, so a caller that disabled the
    // deadline must have it disabled for both passes or the fixture that needs
    // two passes is exactly the one still racing.
    ...(layoutOptions?.budgetMs !== undefined && {
      budgetMs: layoutOptions.budgetMs,
    }),
  };
