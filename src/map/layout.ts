...
export function layoutMap(
  ...
  const hiRef =
    resolved.regions.some((r) => r.iso === 'US-HI') ||
    resolved.pois.some((p) => inHawaii(p.lon, p.lat));
  const { insets, insetRegions, insetLabelSeeds } =
    resolved.projection === 'albers-usa' && usLayer && (akRef || hiRef)
      ? layoutInsets({
          resolved,
          projection,
          usLayer,
      ...
      : { insets: [], insetRegions: [], insetLabelSeeds: [] };
  ...
  // Bow choice is greedy in emit order, so this is one call rather than two:
  // an edge's arc dodges every route leg already placed. See buildConnectorLegs.
  const legs = buildConnectorLegs({
    resolved,
    poiScreen,
    palette,
    width,
  ...
  const usChoroplethZoom =
    ...
    activeIsScore;
  // Text measurement for the label cascades. A factory rather than free
  // functions because `sizeT` needs the canvas's linear extent, and threading
  // that through every call site is how it drifts.
  const labelMetrics = createLabelMetrics(size);
  const { labelW, labelH } = labelMetrics;
  ...
  // Chosen rects also feed `obstacles` so the context-label pass below dodges
  // them too.
  obstacles.push(
    ...layoutEdgeLabels({
      legs,
