...
import { suggestChartTypes, render } from '@diagrammo/dgmo/advanced';
...
const suite = JSON.parse(readFileSync(join(repo, 'tests/fixtures/prompt-suite.json'), 'utf8')).cases;
...
async function generate(row) {
  ...
  let out;
  try {
    out = execFileSync('claude', ['-p', prompt], { encoding: 'utf8', timeout: 120000, maxBuffer: 1 << 20 });
  } catch (e) {
    return { ...row, gen: null, parsed: false, warnings: -1, rendered: false, note: 'gen failed: ' + (e.message || e) };
  }
...
if (live) {
  if (!haveClaude()) {
    console.log('--live requested but `claude` CLI not found — running mechanical only.\n');
  } else {
    liveRows = [];
...
console.log('\n=== DGMO tool scorecard ===\n');
console.log(
  pad('prompt', 46) + pad('chose', 14) + pad('sel', 5) + pad('ref', 4) + pad('ex', 4) + (liveRows ? pad('gen', 5) + 'warn' : ''),
);
console.log('-'.repeat(liveRows ? 86 : 73));
for (const r of R) {
  const sel = r.top1ok ? 'ok' : r.top3ok ? '~3' : 'MISS';
  let line = pad(r.prompt, 46) + pad(r.chosen ?? '-', 14) + pad(sel, 5) + pad(r.hasRef ? 'y' : '-', 4) + pad(r.hasExample ? 'y' : '-', 4);
  if (liveRows) line += pad(r.parsed ? (r.rendered ? 'ok' : 'p-only') : 'FAIL', 5) + (r.warnings >= 0 ? r.warnings : '-');
  console.log(line);
}
...
const summary = {
  ...
  ...(liveRows
    ? {
        ...
        genZeroWarn: pct(liveRows.filter((r) => r.parsed && r.warnings === 0).length, n),
      }
    : {}),
};
console.log('\n=== summary ===');
console.log(`  type selection : top-1 ${summary.selTop1}%   top-3 ${summary.selTop3}%`);
console.log(`  retrieval      : reference ${summary.refCoverage}%   example ${summary.exampleCoverage}%`);
if (liveRows)
  console.log(`  generation     : parse-clean ${summary.genParseClean}%   renders ${summary.genRenderClean}%   zero-warning ${summary.genZeroWarn}%`);

// --- artifacts --------------------------------------------------------------
if (jsonOut) {
  writeFileSync(jsonOut, JSON.stringify({ summary, rows: R.map(({ svg, ...r }) => r) }, null, 2));
...
if (liveRows) {
  ...
  const cards = liveRows
    .map(
      (r) => `<section style="border:1px solid #ccc;border-radius:8px;padding:16px;margin:12px 0">
