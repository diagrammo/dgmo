...
async function stageCliAssets(): Promise<void> {
  ...
  // 🔴 Every directory this function fills is emptied first. Copying into an
  // existing one leaves anything the copy no longer produces sitting there, and
  // `cli/` is gitignored, so nothing shows it: a machine that had built before
  // the TTF-only filter landed kept `cli/fonts/*.woff2` from the old build, and
  // `npm pack` in `cli/` shipped 291,684 bytes the filter was added to remove —
  // upstream Inter, at that, since the stale copies predate the subset. Measured
  // 2026-08-06, one commit after the filter. The same trap catches a renamed or
  // deleted `.claude/commands/*` file, which would ship forever.
  const fill = async (
    dir: string,
    from: string,
    keep: (f: string) => boolean
  ) => {
    await rm(resolve(CLI, dir), { recursive: true, force: true });
    await mkdir(resolve(CLI, dir), { recursive: true });
    for (const f of await readdir(resolve(from))) {
      if (!keep(f)) continue;
      await copyFile(resolve(from, f), resolve(CLI, dir, f));
    }
  };
  ...
  // 🔴 The map assets, or `@diagrammo/dgmo-cli` cannot draw a map at all.
  //
  // `src/map/load-data.ts` resolves them relative to its own bundle — the
  // candidates are `./data`, `./map-data`, `../map-data`, `../src/map/data`.
  // While the CLI lived at `dist/cli.cjs` that found `dist/map-data` for free.
  // Splitting it out to `cli/dist/cli.cjs` (2026-08-06) moved the bundle into a
  // directory where none of the four candidates exist, and because the failure
  // surfaces as the generic "Failed to render diagram" rather than a missing-file
  // error, it read as a bad diagram. Every one of the 15 `map-*` gallery
  // fixtures failed; nothing else did.
  //
  // Copied from `src/map/data` rather than `dist/map-data` so this does not
  // depend on another build config's onSuccess having run first. Absence is
  // tolerated for the same reason `copyMapData` tolerates it: the assets are
  // generated by `pnpm build:map-data` and a non-map build must not hard-fail.
  try {
    await fill('dist/map-data', './src/map/data', (f) => f.endsWith('.json'));
  } catch {
    /* map data not generated — `pnpm build:map-data` produces it */
  }
