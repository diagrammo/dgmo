...
describe('isWorkday', () => {
  it('Monday is a workday', () => {
    expect(
      isWorkday(makeDate(2024, 1, 15), [...DEFAULT_WORKWEEK], new Set())
    ).toBe(true);
  });

  it('Saturday is not a workday', () => {
    expect(
      isWorkday(makeDate(2024, 1, 20), [...DEFAULT_WORKWEEK], new Set())
    ).toBe(false);
  });

  it('Sunday is not a workday', () => {
    expect(
      isWorkday(makeDate(2024, 1, 21), [...DEFAULT_WORKWEEK], new Set())
    ).toBe(false);
  });
  ...
  it('holiday is not a workday', () => {
    ...
    expect(
      isWorkday(makeDate(2024, 1, 15), [...DEFAULT_WORKWEEK], holidays)
    ).toBe(false);
  });
});
...
describe('buildHolidaySet', () => {
  it('expands date ranges', () => {
    const holidays: GanttHolidays = {
      ...
      ranges: [
        { startDate: '2024-12-24', endDate: '2024-12-26', label: 'Xmas' },
      ],
      workweek: [...DEFAULT_WORKWEEK],
    };
...
describe('formatGanttDate — datetime support', () => {
  ...
  it('returns YYYY-MM-DD HH:MM for non-midnight', () => {
    expect(formatGanttDate(new Date(2024, 5, 15, 14, 30))).toBe(
      '2024-06-15 14:30'
    );
  });
...
describe('sprint duration (s unit)', () => {
  const emptyHolidaySet = new Set<string>();

  it('2s with sprint-length 2w = 28 calendar days', () => {
    ...
    const end = addGanttDuration(
      ...
      emptyHolidaySet,
      1,
      { sprintLength: { amount: 2, unit: 'w' } }
    );
    expect(fmt(end)).toBe('2026-02-02');
  });

  it('0.5s with sprint-length 2w = 7 calendar days', () => {
    const start = makeDate(2026, 1, 5);
    const end = addGanttDuration(
      start,
      { amount: 0.5, unit: 's' },
    ...
    expect(fmt(end)).toBe('2026-01-12');
  });

  it('1.5s with sprint-length 3w = 32 calendar days (rounding)', () => {
    const start = makeDate(2026, 1, 5);
    const end = addGanttDuration(
      start,
      { amount: 1.5, unit: 's' },
      ...
      emptyHolidaySet,
      1,
      { sprintLength: { amount: 3, unit: 'w' } }
    );
    // 1.5 * 3 * 7 = 31.5 → Math.round(31.5) = 32 days
    expect(fmt(end)).toBe('2026-02-06');
  });

  it('throws if sprintLength is not provided', () => {
    const start = makeDate(2026, 1, 5);
    expect(() =>
      addGanttDuration(
        start,
    ...
    ).toThrow('sprintLength');
  });

  it('parseDuration parses 2s', () => {
    expect(parseDuration('2s')).toEqual({ amount: 2, unit: 's' });
  });

  it('parseDuration parses 0.5s', () => {
  ...
  it('2s with sprint-length 10d = 20 calendar days', () => {
    ...
    const end = addGanttDuration(
      ...
      emptyHolidaySet,
      1,
      { sprintLength: { amount: 10, unit: 'd' } }
    );
    expect(fmt(end)).toBe('2026-01-25');
  });
});
