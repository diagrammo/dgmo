...
describe('hex / CSS colors are rejected with an error diagnostic', () => {
  ...
  it('rgb()/hsl() literals also error', () => {
    for (const literal of ['rgb(255,0,0)', 'rgba(0,0,0,0.5)', 'hsl(0,100%,50%)']) {
      const diagnostics: DgmoError[] = [];
...
[North America] #e6194b
  United States 76300 12700`;
    ...
    expect(errors.some((d) => /hex or CSS color values/.test(d.message))).toBe(true);
...
Whites s: White
  Chardonnay`;
    const { diagnostics } = parseDgmo(src);
    expect(diagnostics.filter((d) => d.code === INVALID_COLOR_CODE)).toHaveLength(
      0
    );
  });
