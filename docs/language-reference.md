...
| `no-legend`    | Hide the legend and collapse the vertical band reserved for it — accepted by **every** chart type that renders a legend |
...
- **No `end`/terminator keyword** — a block closes by dedenting (like multi-line notes). `end` and `activate`/`deactivate` are not dgmo syntax (a bare `end` line is rejected); activation is controlled by the `activations` option (§2.7), not per-message keywords.
...
- `autonumber` — prefix every authored message with its stable source-order number. Bare messages show only the number; inferred returns stay unnumbered. Numbers can skip when folding hides a message, so references made in discussion do not change.
...
sitemap are excluded — their indentation _is_ the tree structure, which
...
- Legend shows by default whenever the chart is multi-series; suppress it with
  the universal `no-legend` flag (§1.8). Single-series charts render no legend,
  and take the flag as a no-op.
...
- `now` / `now <date>` — plant a **"now" pin** on the spine: a red diamond at today with a short stem to a labeled tab that auto-slots into a card-free lane. Bare `now` is **computed** (resolved to today each render); `now <date>` **pins** it to an explicit ISO date. **The tab is captioned with that date** — `Aug 11, 2026`, formatted exactly as the event cards format theirs — so a reader can tell a diagram that redraws from a picture of one that stopped; a trailing token replaces the caption (`now 2026-01-01 Launch`). A trailing **named color** recolors the pin off its default red (`now blue`, `now 2026-01-01 Launch purple`) — reach for it when the timeline's own tags already use red, so the pin stops standing apart from them. The tab honors the fill family (`fill-solid`/`fill-outline`/tint). To-scale only — ignored under `no-scale`. Positioned over real dated events (`TBD` positions never anchor it); a `now` past the last date rides onto the open horizon when a trailing `TBD` is present, else clamps to the last dot.
...
- `size:` scales marker area (use `poi-size <label>` for the legend key). A trailing color (`poi Austin red`) sets the marker fill directly — winning over a tag color and over the fallback fill. A POI that names no value for ANY declared tag group takes that group's default value (the entry marked `default`, else the first) and is colored as that category — DGMO's universal convention, the same assumption a region makes. That includes a group used solely on connector lines: a `Leg` group of Sail/March/Row paints every untagged harbor `Sail`. **To keep untagged markers reading as untagged, declare that case as the group's first entry** (`NA gray`, `Untagged gray`) or mark another entry `default` — the renderer never invents an untagged category. A marker with nothing left to fall back to paints the palette neutral (`gray`), and orange only where no tag group is declared at all. POI properties: `label`, `size`, `style`, applied tag alias, `as`, `clock`. No `icon` in v1.
...
- **The place picks the zone — don't type it.** A named city derives its IANA zone from the gazetteer (correct by construction, e.g. Austin → Central). Only a **bare-coordinate** pin needs the valued form `clock: <zone>` — an IANA id (`Asia/Tokyo`) or a fixed offset (`clock: UTC+9`, no DST). The valued form also _overrides_ a city, but a mismatch warns (you almost never want it).
...
- **A city name must match the gazetteer's canonical form** — `New York City`, not `New York`; `Washington, D.C.`, not `Washington`. An unrecognized name is dropped (with a did-you-mean note) and the map reframes around the places that _did_ resolve, so the result can look complete but be missing a point. Use the exact name or coordinates.
