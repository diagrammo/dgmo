...
- `now` / `now <date>` — plant a **"now" pin** on the spine: a red diamond at today with a short stem to a labeled tab that auto-slots into a card-free lane. Bare `now` is **computed** (resolved to today each render); `now <date>` **pins** it to an explicit ISO date. **The tab is captioned with that date** — `Aug 11, 2026`, formatted exactly as the event cards format theirs — so a reader can tell a diagram that redraws from a picture of one that stopped; a trailing token replaces the caption (`now 2026-01-01 Launch`). The tab honors the fill family (`fill-solid`/`fill-outline`/tint). To-scale only — ignored under `no-scale`. Positioned over real dated events (`TBD` positions never anchor it); a `now` past the last date rides onto the open horizon when a trailing `TBD` is present, else clamps to the last dot.
