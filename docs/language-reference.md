...
- `size:` scales marker area (use `poi-size <label>` for the legend key). A trailing color (`poi Austin red`) sets the marker fill directly — winning over a tag color and over the fallback fill. A POI that names no value for a tag group other POIs use takes that group's default value (the entry marked `default`, else the first) and is colored as that category — the same assumption a region makes; a group used solely on connector lines never colors a marker. A marker with nothing left to fall back to paints the palette **neutral** (`gray`) on any map that declares a tag group, and **orange** only on a map that declares none — orange is a color an author may declare as a tag value, so an untagged marker wearing it would be indistinguishable from a tagged one. A POI naming a value its group never declares takes that same neutral. POI properties: `label`, `size`, `style`, applied tag alias, `as`, `clock`. No `icon` in v1.
