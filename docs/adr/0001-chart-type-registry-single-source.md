...
dgmo has ~45 chart types. Each needs to be known to several subsystems: the parser dispatch, the export-handler map, the dimensions/sizing logic, the public `chartTypes` list, and the CLI `types` command. If each subsystem kept its own list, adding a chart type would mean editing N places and the lists would drift.

## Decision

`src/chart-type-registry.ts` holds `CHART_TYPE_REGISTRY` — one `ChartTypeDescriptor` per chart type (`{ id, category, parse, measure?, minDims? }`) — and the derived `REGISTRY_BY_ID` map. Every other table derives from it or is cross-checked against it by `chart-type-registry.test.ts` (registry ↔ `chartTypes` ↔ `DIAGRAM_EXPORT_HANDLERS` ↔ dimensions must stay in sync, enforced as a test).
