...
- **Homebrew** bundles the MCP server, so `brew upgrade dgmo` upgrades the CLI _and_ the server together. Re-run `dgmo install` afterward only to refresh the **skill files** (those are copies in your home dir).
